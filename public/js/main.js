(function($) {
  ...
  function mobileMenuInit() {
    ...
    $(window).on("resize", function() {
      if($(window).width() > 1200 && $body.hasClass('not-scroll')) {
        hide();
      }
    });
  ...
  function customRangeSlider(t) {
    ...
    sl.each(function() {
      ...
      minValue.on('blur', function() {
        var
          input    = $(this),
          newValue = _.toNumber(input.val()),
          dataMin  = _.toNumber(_.isNaN(slider.attr('data-slider-min')) ? 0 : slider.attr('data-slider-min')),
          dataMax  = _.toNumber(_.isNaN(slider.attr('data-slider-max')) ? 0 : slider.attr('data-slider-max'));

        var
          valArray    = slider.val().split(','),
          oldMaxValue = valArray[1];
        ...
        if(!dataMin)
          slider.attr('data-slider-min', valArray[0]);

        slider.slider('setValue', [_.toNumber(newValue), _.toNumber(oldMaxValue)], true);
      });
      ...
      maxValue.on('blur', function() {
        ...
        if(newValue)
          slider.slider('setValue', [_.toNumber(oldMinValue), _.toNumber(newValue)], true);
      });
    });
  }
