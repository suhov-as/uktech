...
class MainController extends Controller
{
    ...
    private function _catalog_array($name)
    {
        $filters = [
            ...
            'rent' => [
                ...
                'group' => [
                    'group_1_ASC'  => 'price_money',
                    'group_1_DESC' => 'price_money',
    ...
    public function friend_form()
    {

        $params['objects'] = $this->dynamic->t('catalog_new_building')
            ->join(
                'files',

                function ($join) {
                    $join->type = 'LEFT OUTER';
                    $join->on("catalog_new_building.id", '=', 'files.id_album')
                        ->where('files.name_table', '=', "catalog_new_buildingalbum")
                        ->where('files.main', '=', 1);
                }
            )
            ->select("catalog_new_building.*", 'files.file', 'files.crop')
            ->get()
            ->toArray();
...
//      );
        ...
        return $this->base->view_s('emails.friend_form', $params);
    }
            ...
            foreach ($cart_data as $k => $ids) {
                ...
                if ($filters) {
                    ...
                    foreach ($catalog as $cat) {
                        $objects[] = array_merge($cat, ['name_url' => $k]);
                    }
                }
            }
    ...
    public function search_render_catalog()
    {
        ...
        if ($form['name_url'] && !$session) {
            ...
            if (isset($form['area_from_fixed']) && isset($form['area_to_fixed'])) {
                if (($form['type_ft_m2'] ?? false) === 'ft') {
